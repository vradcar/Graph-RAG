...
def ingest_replacements(
    ...
    # 2) Nodes + edges in a separate session/transaction.
    product_count = 0
    edge_count = 0
    skipped = 0
    with driver.session() as session:
        ...
        for rep in replacements:
            ...
            if old_id not in therms_by_id or new_id not in therms_by_id:
                log.warning("Replacement endpoints missing in thermostats[]: %s -> %s", old_id, new_id)
                skipped += 1
                continue
            ...
            edge = {
                "source_id": new_id,
                "target_id": old_id,
                "relation": "REPLACES",  # in ALLOWED_RELATIONS
                "properties": {
